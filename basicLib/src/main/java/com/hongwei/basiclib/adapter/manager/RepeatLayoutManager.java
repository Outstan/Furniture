package com.hongwei.basiclib.adapter.manager;
 ...
 * @Describe :自定义LayoutManager 使RecycleView实现无限循环卡片
 */
public class RepeatLayoutManager extends RecyclerView.LayoutManager {
    ...
    private void layoutChunk(RecyclerView.Recycler recycler) {
        if (mOrientation == RecyclerView.HORIZONTAL) {
            //横向滑动
            //获取RecyclerView的paddingleft
            int itemLeft = getPaddingLeft();
            for (int i = 0; ; i++) {
                //如果当前item左边距离 > RecyclerView宽度(除去margin的) - RecyclerView的getPaddingRight
            ...
            //垂直滑动
            //获取RecyclerView的getPaddingTop
            int itemTop = getPaddingTop();
            for (int i = 0; ; i++) {
                //如果当前item上边边距离 > RecyclerView高度(除去margin的) - RecyclerView的getPaddingBottom
